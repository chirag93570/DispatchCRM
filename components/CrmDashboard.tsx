...
import { DollarSign, TrendingUp, Users, Activity, X, Phone, MapPin, Truck, Save, Clock, History, Send } from 'lucide-react';
...
const CrmDashboard: React.FC<Props> = ({ stats, leads, onLeadSelect, onQuickCall, onRefresh, activeLeadId }) => {
    ...
    // Sync selected lead if activeLeadId changes externally
    useEffect(() => {
        if (activeLeadId) {
    ...
    const handleLeadClick = (lead: Lead) => {
        ...
        onLeadSelect(lead); // Notify parent but stay on dashboard
        setNoteContent(''); // Reset note input
    };
    ...
    return (
        <div className="h-full flex flex-col p-6 overflow-hidden bg-gray-950">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 shrink-0 mb-8">
                <KpiCard label="Pipeline Value" value="$124,500" sub="12 Deals closing soon" icon={DollarSign} color="bg-green-600" />
                <KpiCard label="Win Rate" value="22%" sub="+4% vs last month" icon={TrendingUp} color="bg-blue-600" />
                <KpiCard label="New Leads" value={stats.leadsInQueue} sub="Action Required" icon={Users} color="bg-purple-600" />
                <KpiCard label="Activity" value="High" sub="45 actions today" icon={Activity} color="bg-orange-600" />
            </div>
            ...
            <div className="flex-1 min-h-0 flex gap-6">
                ...
                {/* Right: Lead Detail Panel (Conditional) */}
                {selectedLead && (
